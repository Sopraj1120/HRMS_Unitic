...
namespace HRMS.Repository
{
    public class UserRepository : IUserRepo
    {
        ...
        {
            var data = await _context.users.Where(a => !a.IsDeleted).Include(a => a.userAddresses).Include(e => e.userHigherStudies).ToListAsync();
            return data;
        }

        public async Task<Users> GetUserById(Guid Id)
        {
            var data = await _context.users.Include(A =>A.userAddresses).Include(e => e.userHigherStudies).FirstOrDefaultAsync(x =>x.Id == Id && !x.IsDeleted);
            return data;
        }
