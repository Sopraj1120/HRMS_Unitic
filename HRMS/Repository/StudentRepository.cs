...
namespace HRMS.Repository
{
    public class StudentRepository : IStudentRepo
    {
        ...
        {
            var data = await _context.Students.Where(x => !x.IsDeleted).Include(p => p.Parents).Include(a => a.Address).Include(o => o.OLs).Include(a => a.ALs).ToListAsync();
            return data;
        }

        public async Task<Students> GetStudentById(Guid Id)
        {
            var data = await _context.Students.Include(p => p.Parents).Include(a => a.Address).Include(o => o.OLs).Include(a => a.ALs).FirstOrDefaultAsync(a => a.Id == Id && !a.IsDeleted);
            return data;
        }
